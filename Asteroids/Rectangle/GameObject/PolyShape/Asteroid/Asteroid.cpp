...
namespace Asteroids {
    Asteroid::Asteroid(float x, float y, float angle, float dx, float dy, float dAngle, float size, ParticleSystem* particleSystem)
        : particleSystem(particleSystem), PolyShape(x, y, angle, dx, dy, dAngle, size, std::vector<std::pair<float, float>>()) {
